...
def mergeG(gA, gB):
    # For example given frames A 1 2 and B 2 3
    # then cameras are frames 
    ...
    # if there are no common then error
    # if there is no proper B then error
    if len(comFram) == 0:
        raise Exception("Common empty ")
    if len(propB) == 0:
        raise Exception("there is no proper B")

    # greate a mix graph similar to grpah A
    merged = deepcopy(gA)

    # For the first overlap (it coul have many)
    firstOv = comFram[0]

    # Transform B.mot b.str to A coordinate system
    commonA = gA.frames.index(firstOv)
    commonB = gB.frames.index(firstOv)
    # transform rtB

    transf = catRt(invertRt(gA.mot[:, :, commonA]), gB.mot[:, :, commonB])
    gB.str = transformPtsByRt(np.transpose(gB.str), transf, False)  # Apply to str B

    # Mot now is the concatenation of mot and RtB^-1
    ...
    merged.mot = newMot
    # Agragates frames to graph

    # common case withmore than one frame
    for fr in comFram:
        ...
        for i in range(iA.shape[0]):
            ...
            for j in range(len(indpB)):
                ...
                # Agregate an element to obsVal and ObsIdx
                print(merged.obsVal)
    ...
    #Reviewing if there is any column without value
    #Select in ObsIdx common frames and dif en Gb
    #Siendo A = common columns with value of -1
    #Siendo B = dif columns with sum of the values more than -1 is more than 0
...
def removeOutlierPts(g, th_pix=10):
    ...
    for i in range(g.ObsIdx.shape[1]):
        ...
        if cantB > 0:
            print ("deleted ", cantB, " outliers of ", outliers.shape[0], \
                " total points with  sq_th_pix of ", sq_th_pix)
    ...
    for c1 in range(g.ObsIdx.shape[1]):
        for c2 in range(g.ObsIdx.shape[1]):
            ...
            if cantB > 0:
                print ("deleted ",cantB," outliers of ",outliers.shape[0],\
                    " total points with cost_thr of ",tincos)
            p2keep = np.ones((1, g.str.shape[0]))
            p2keep[:,selector[outliers]] = False
            p2keep = p2keep[0].astype(np.bool)
            g.str = g.str[p2keep, :]
